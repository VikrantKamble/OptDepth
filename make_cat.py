...
for k in range(nObj):
	...
	try:
		...
		s = np.floor((log2rest - COEFF0)/COEFF1).astype(int)
		temp1 = where((s >= 0) & (s < 8140))[0]
		...
		minT, maxT = np.floor((minSky - COEFF0)/COEFF1).astype(int), np.ceil((maxSky - COEFF0)/COEFF1).astype(int)
...
hdict = {'COEFF0': COEFF0, 'COEFF1': COEFF1, 'NPIX': NPIX, 'AUTHOR': X['author']}
